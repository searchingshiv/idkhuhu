...
async def ds_link(_id):
    ...
    if "video" in mime_type:
        stream_text = LANG.DS_TEXT.format(file_name, file_size, stream_link)
        reply_markup = InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("ᴅᴏᴡɴʟᴏᴀᴅ", url=stream_link)],
                [InlineKeyboardButton("ᴄʟᴏsᴇ", callback_data="close")]
...
# Use If Facing Issue In Force Sub While Using Link Command In Group !!   

# async def group_verify(bot, message):
#     if await is_user_banned(message):
#         return False
